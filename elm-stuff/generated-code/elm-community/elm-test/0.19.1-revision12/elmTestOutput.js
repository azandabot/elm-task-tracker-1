...
var $author$project$Model$initModel = {newTaskName: '', tasks: _List_Nil};
...
var $author$project$First_Test$initModelTests = A2(
	...
	_List_fromArray(
		[
			...
			'has an empty task list',
			function (_v0) {
				return A2($elm_explorations$test$Expect$equal, $author$project$Model$initModel.tasks, _List_Nil);
			}),
			A2(
			$elm_explorations$test$Test$test,
			'has an empty newTaskName',
			function (_v1) {
				return A2($elm_explorations$test$Expect$equal, $author$project$Model$initModel.newTaskName, '');
			})
		]));
...
var $author$project$Update$AddTask = {$: 'AddTask'};
var $author$project$Update$RemoveTask = function (a) {
	return {$: 'RemoveTask', a: a};
};
var $author$project$Update$ToggleTask = function (a) {
	return {$: 'ToggleTask', a: a};
};
var $author$project$Update$removeTask = F2(
	function (name, tasks) {
		return A2(
			$elm$core$List$filter,
			function (task) {
				return !_Utils_eq(task.name, name);
			},
			tasks);
	});
var $author$project$Update$toggleTask = F2(
	function (name, tasks) {
		return A2(
			$elm$core$List$map,
			function (task) {
				return _Utils_eq(task.name, name) ? _Utils_update(
					task,
...
var $author$project$Update$update = F2(
	function (msg, model) {
		switch (msg.$) {
			case 'NewTaskNameInput':
				var newName = msg.a;
				return _Utils_update(
					model,
					{newTaskName: newName});
			case 'AddTask':
				return $elm$core$String$isEmpty(model.newTaskName) ? model : _Utils_update(
					model,
					{
						newTaskName: '',
						tasks: A2(
							$elm$core$List$cons,
							{completed: false, name: model.newTaskName},
							model.tasks)
					});
			case 'ToggleTask':
				var taskName = msg.a;
				return _Utils_update(
					model,
					{
						tasks: A2($author$project$Update$toggleTask, taskName, model.tasks)
					});
			default:
				var taskName = msg.a;
				return _Utils_update(
					model,
					{
						tasks: A2($author$project$Update$removeTask, taskName, model.tasks)
					});
		}
	});
var $author$project$First_Test$updateTests = A2(
	$elm_explorations$test$Test$describe,
	'update',
	_List_fromArray(
		[
				...
				var model = _Utils_update(
					$author$project$Model$initModel,
					{
						tasks: _List_fromArray(
							[
								{completed: false, name: 'Task 1'}
							])
					});
				var updatedModel = A2(
					$author$project$Update$update,
					$author$project$Update$ToggleTask('Task 1'),
					model);
				return A2(
				...
				var model = _Utils_update(
					$author$project$Model$initModel,
					{
						tasks: _List_fromArray(
							[
								{completed: false, name: 'Task 1'},
								{completed: false, name: 'Task 2'}
							])
					});
				var updatedModel = A2(
					$author$project$Update$update,
					$author$project$Update$RemoveTask('Task 1'),
					model);
				return A2(
					$elm_explorations$test$Expect$equal,
					updatedModel.tasks,
...
var $author$project$Test$Generated$Main$main = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$First_Test$initModelTests),
					$author$project$Test$Runner$Node$check($author$project$First_Test$updateTests)
				]))
		]));
